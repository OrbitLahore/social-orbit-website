...
function upcomingCard(ev, delay) {
  const lvl    = escapeHTML(select(ev.properties.Level)       || 'LVL_01');
  const name   = escapeHTML(title(ev.properties.Name));
  const desc   = escapeHTML(text(ev.properties.Description));
  const d      = escapeHTML(fmtDate(date(ev.properties.Date)));
  const time   = escapeHTML(text(ev.properties.Time)          || 'TBA');
  const loc    = escapeHTML(text(ev.properties.Location)      || 'TBA');
  const slug   = escapeHTML(text(ev.properties.Event_Slug)    || 'event');
  ...
  return `
      ...
      <div class="pixel-card reveal" style="--d:${delay}s">
        ...
        <div style="border-bottom:2px solid var(--pixel-violet);padding-bottom:12px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:center">
          ...
          <span style="font-family:var(--font-mono);font-size:10px;color:${hpColor}">HP: ${escapeHTML(hp)}</span>
        </div>
        ...
        <a href="${escapeHTML(extUrl)}" class="pixel-btn" style="width:100%;justify-content:center;text-align:center;display:block">Join_Queue →</a>
      </div>`;
}
...
<head>
  ...
  <meta name="description" content="Upcoming Social Orbit events in Lahore. One room. Zero pressure.">
...
<section class="events-hero">
  ...
  <div class="events-hero-inner">
    ...
    <p class="pixel-body reveal" style="--d:.1s;max-width:480px;">
      one room. zero pressure. pick your level.
    </p>
  </div>
</section>
    ...
    <div class="steps-grid">
      ...
      <div class="step-item reveal" style="--d:.15s">
        ...
        <div class="step-content">
          ...
          <p>no business cards. just people who showed up for the same reason.</p>
        </div>
      </div>
    </div>
  </div>
